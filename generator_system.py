...
# Шаблоны JSON (без изменений структуры, но мы добавим поле theme программно)
TEMPLATES = {
    "single": """{
    "type": "single",
...
def clean_json_response(text: str) -> str:
    ...
    if text.startswith("```json"):
        text = text[7:]
    elif text.startswith("```"):  # Исправлено условие
        text = text[3:]
    if text.endswith("```"):
        text = text[:-3]
    return text.strip()
...
def generate_questions_with_context(context_data):
    ...
    for q_type in context_data['q_types']:
        count = 3  # Оптимально для теста, можно увеличить
        logger.info(f"🚀 Генерация: Тема '{topic}' -> Тип '{q_type}'")

        # Улучшенный системный промпт
        prompt = f"""
        Ты — креативный учитель обществознания и права для детей и подростков (8-16 лет). 
        Твоя цель — превратить скучный юридический текст в увлекательные задачки.

        ВХОДНОЙ ТЕКСТ (ИСТОЧНИК):
        "{legal_text}"

        ЗАДАЧА: Создать {count} вопросов типа "{q_type}" в формате JSON.

        СТИЛЬ И ПОДАЧА:
        1. **Ситуативность**: Используй имена (Вася, Маша), конкретные места (школа, парк, соцсети).
        2. **Простота**: Избегай канцеляризмов ("осуществляет деятельность", "в соответствии с пунктом"). Заменяй их на простые слова.
        3. **Юмор**: Допускается легкий, уместный юмор в вариантах ответов (особенно в неправильных).
        4. **Обучение**: Даже неправильные ответы должны учить отличать правду от мифа.

        ТЕХНИЧЕСКИЕ ТРЕБОВАНИЯ:
        1. Формат JSON массива: [{TEMPLATES[q_type]}, ...].
        2. Язык: Русский.
        3. Поле "hint" (Подсказка): Должно наводить на мысль, но не говорить ответ прямо. Используй эмодзи (💡, 👉).
        4. Строго соблюдай структуру JSON шаблона выше.
        ...
        # Используем json_object response format если модель поддерживает, но versatile надежнее текстом
        data = {
            "model": MODEL,
            "messages": [
                {"role": "system", "content": "Ты JSON-генератор. Отвечай только валидным JSON."},
        ...
        while retries < MAX_RETRIES and not success:
            try:
                ...
                if response.status_code == 429:
                    ...
                    logger.warning(f"⚠️ Лимит (429). Ждем {wait_time}с...")
                ...
                # ПОСТОБРАБОТКА: Внедряем тему и ID
                for q in result_json:
                    q['theme'] = topic  # <-- ВАЖНО: Добавляем тему программно
                    # Генерируем уникальный хэш для дедупликации
                    q_hash = hashlib.md5((q['question'] + q.get('type', '')).encode()).hexdigest()
                    q['id'] = q_hash
                ...
                logger.error(f"❌ Ошибка ({q_type}): {e}")
                retries += 1
                time.sleep(2)
...
def main():
    ...
    # Загрузка и дедупликация существующих
    if os.path.exists(OUTPUT_FILE):
        try:
            with open(OUTPUT_FILE, 'r', encoding='utf-8') as f:
                loaded = json.load(f)
                for q in loaded:
                    # Если есть ID, используем его, иначе генерируем
                    qid = q.get('id', hashlib.md5((q['question']).encode()).hexdigest())
                    if qid not in existing_ids:
                        all_questions.append(q)
                        existing_ids.add(qid)
                logger.info(f"📚 Загружено {len(all_questions)} уникальных вопросов.")
        except Exception as e:
            logger.error(f"Ошибка чтения файла: {e}")

    # Генерация новых
    for context_entry in CONTEXT_REGISTRY:
        new_batch = generate_questions_with_context(context_entry)
        added_count = 0
        ...
        if added_count > 0:
            logger.info(f"✅ Добавлено {added_count} новых вопросов по теме '{context_entry['topic']}'")
            with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
                json.dump(all_questions, f, ensure_ascii=False, indent=2)

    logger.info(f"\n🎉 Всего вопросов в базе: {len(all_questions)}")
