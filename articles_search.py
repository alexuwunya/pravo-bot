...
async def enable_notifications(callback: types.CallbackQuery):
    ...
    success = news_db.set_notification_status(user_id, True)
...
async def disable_notifications(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    news_db.set_notification_status(user_id, False)
    ...
    while True:
        try:
            ...
            if update_result['status'] == 'updated' and update_result['new_articles'] > 0:
                users = news_db.get_users_with_notifications()
                ...
                for user_id, last_notified_date in users:
                    try:
                        ...
                        if new_articles:
                            ...
                            news_db.update_last_notified_date(user_id)
...
async def back_to_main_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    user_id = callback.from_user.id
    notifications_enabled = news_db.get_notification_status(user_id)
